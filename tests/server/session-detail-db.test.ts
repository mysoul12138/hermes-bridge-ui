...
describe('session DB detail', () => {
  ...
  it('reconstructs compressed continuation messages for session detail', async () => {
    ...
    const continuationDetail = await mod.getSessionDetailFromDb('root-cont')

    expect(continuationDetail?.id).toBe('root-cont')
    expect(continuationDetail?.started_at).toBe(100)
    expect(continuationDetail?.thread_session_count).toBe(2)
    expect(continuationDetail?.messages.map((message: any) => `${message.session_id}:${message.role}:${message.content}`)).toEqual([
      'root:user:before compression',
      'root:assistant:',
      'root-cont:tool:{"output":"/tmp"}',
      'root-cont:assistant:after compression',
