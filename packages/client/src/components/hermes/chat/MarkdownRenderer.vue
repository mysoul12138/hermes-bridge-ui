...
const renderedHtml = computed(() => {
  ...
  html = html.replace(/src="\/([^"]+)"/g, (_match, path) => {
    ...
    return `src="${escapeHtmlAttr(downloadUrl)}"`
  })
  ...
  html = html.replace(/<a href="(\/[^"]+)">([^<]+)<\/a>/g, (match, path, filename) => {
    ...
    if (ext === 'mp4' || ext === 'webm' || ext === 'mov') {
      ...
      return `<div class="markdown-video-container">
        <video class="markdown-video" controls preload="metadata" src="${safeDownloadUrl}"></video>
    ...
    const safeFileNameAttr = escapeHtmlAttr(fileName)
    const safeFileName = escapeHtmlText(fileName)
    const safeTitle = escapeHtmlAttr(t('download.downloadFile'))
    return `<div class="markdown-file-card" data-path="${safePath}" data-filename="${safeFileNameAttr}" title="${safeTitle}">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
